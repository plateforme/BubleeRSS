...
function creerFlux(titre) {
  ...
  ).run('https://presse.test/' + titre + '/rss', titre, Date.now()).lastInsertRowid);
}
...
test('un guid deja connu ne cree pas de seconde ligne', () => {
  ...
  assert.deepEqual(store.saveItems(fluxA, [item]), { ajoutes: 1, doublons: 0 });
  assert.deepEqual(store.saveItems(fluxA, [item]), { ajoutes: 0, doublons: 0 });
...
test('meme flux, guid change, meme adresse : rien n est duplique', () => {
  ...
  store.saveItems(fluxA, [article({ guid: 'v1', url, title: 'Un article republie par son editeur maladroit' })]);
...
test('deux flux relaient la meme adresse : la copie est rattachee a l originale', () => {
  ...
  store.saveItems(fluxA, [article({ guid: 'a-reprise', url, title: titre })]);
  const resultat = store.saveItems(fluxB, [article({ guid: 'b-reprise', url: url + '?utm_medium=rss', title: titre })]);
...
test('titre identique et dates proches suffisent quand l adresse differe', () => {
  ...
  store.saveItems(fluxA, [article({ guid: 'a-pont', url: 'https://a.test/pont', title: titre })]);
...
test('un titre court ne suffit pas a declarer un doublon', () => {
  store.saveItems(fluxA, [article({ guid: 'a-court', url: 'https://a.test/breve', title: 'Revue de presse' })]);
  const resultat = store.saveItems(fluxB, [article({ guid: 'b-court', url: 'https://b.test/breve', title: 'Revue de presse' })]);
...
test('un titre identique mais publie bien plus tard reste un article distinct', () => {
  ...
  store.saveItems(fluxA, [article({ guid: 'a-hebdo', url: 'https://a.test/h1', title: titre })]);
  const resultat = store.saveItems(fluxB, [article({
    guid: 'b-hebdo',
    url: 'https://b.test/h2',
    title: titre,
...
test('les doublons sont masques globalement mais visibles dans leur flux', () => {
  ...
  store.saveItems(fluxA, [article({ guid: 'a-masque', url, title: titre })]);
  store.saveItems(fluxB, [article({ guid: 'b-masque', url, title: titre })]);

  const global = store.queryArticles({ view: 'all', limit: 200 }).articles;
  assert.equal(global.filter((a) => a.title === titre).length, 1);

  const dansB = store.queryArticles({ view: 'all', feedId: fluxB, limit: 200 }).articles;
...
test('lire un exemplaire marque toute l histoire comme lue', () => {
  ...
  store.saveItems(fluxA, [article({ guid: 'a-lect', url, title: titre })]);
  store.saveItems(fluxB, [article({ guid: 'b-lect', url, title: titre })]);

  const original = db.prepare('SELECT id FROM articles WHERE guid = ?').get('a-lect').id;
  store.setRead(original, true);
...
test('une copie arrivee apres coup herite de l etat de lecture', () => {
  ...
  store.saveItems(fluxA, [article({ guid: 'a-apres', url, title: titre })]);
  store.setRead(db.prepare('SELECT id FROM articles WHERE guid = ?').get('a-apres').id, true);

  store.saveItems(fluxB, [article({ guid: 'b-apres', url, title: titre })]);
...
test('le compteur global ne compte chaque histoire qu une fois', () => {
  ...
  store.saveItems(fluxA, [article({ guid: 'a-cpt', url, title: titre })]);
  store.saveItems(fluxB, [article({ guid: 'b-cpt', url, title: titre })]);
...
test('marquer tout lu propage aux copies des autres flux', () => {
  ...
  store.saveItems(fluxA, [article({ guid: 'a-tout', url, title: titre })]);
  store.saveItems(fluxB, [article({ guid: 'b-tout', url, title: titre })]);

  store.markRead({ feedId: fluxA });
...
test('dans un meme flux, une adresse partagee ne replie pas des titres differents', () => {
  ...
  store.saveItems(fluxA, [article({ guid: 'em1', url: lien, title: 'Le premier episode parle longuement de Spinoza' })]);
  const resultat = store.saveItems(fluxA, [
